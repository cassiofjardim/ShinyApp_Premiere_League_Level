...
stacked_bar_chart_function <- function(df,x_axis,y_axis,y2_axis){
...
bar_chart_function <- function(df,x_axis,y_axis,y2_axis){
  df %>%
  ...
  hchart(
    ...
    hc_size( height = 250, width = 'auto') %>%
    hc_boost(enabled = TRUE)%>%
    hc_credits(enabled = TRUE) 
  
}

bar_chart <- function(df,x_axis,y_axis,color, input = NULL){
  df %>%
  hchart(
    'column',
    names = 'SQUADS',
